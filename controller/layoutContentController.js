...
const getLayoutContent = asyncHandler(async (req, res) => {
    const content = await LayoutContent.findOne().populate({
      ...
      select: "imageLinks.original imageLinks.thumbnail photographer resolutions title description story keywords category photographer license price location cameraDetails",
      populate: {
        path: 'photographer'
      }
    });
