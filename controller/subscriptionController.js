...
const createSubscription = asyncHandler(async (req, res) => {
  ...
  const selectedCost = plan.cost.find(
    (cost) => cost.duration === duration && cost.price === price
  );

  if (!selectedCost) {
    return res
      .status(400)
      .json({ message: "Invalid price or duration selected" });
  }
...
const getUserActiveSubscription = asyncHandler(async (req, res) => {
  ...
  if (!subscription) {
    return res
      .status(400)
      .send({ message: "No Active Subscription found for user" });
  }
...
const checkAndSendSubscriptionEmails = asyncHandler(async () => {
  ...
  const subscriptions = await Subscription.find({
    endDate: { $gte: startRange, $lt: endRange },
    isActive: true,
  ...
  const emailPromises = subscriptions.map(async (subscription) => {
    ...
    await sendMembershipRenewalReminderMail(
      photographerName,
      photographerEmail
    );
  });
...
const weeklyMailToInactivePhotographers = asyncHandler(async (req, res) => {
  ...
  const emailPromises = photographersWithoutImages.map(async (photographer) => {
    ...
    await sendWeeklyMailToInactivePhotographers(
      photographerName,
      photographerEmail
    );
  });
...
const weeklyMailToNonMonetizedPhotographers = asyncHandler(async (req, res) => {
  ...
  const emailPromises = nonMonetizedPhotographers.map(async (photographer) => {
    const photographerName = `${photographer.firstName} ${photographer.lastName}`;
    const photographerEmail = photographer.email;
    await sendWeeklyMailToNonMonetizedPhotographers(
      photographerName,
      photographerEmail
    );
    try {
      await sendNotificationToUser({
        ...
        title: "Monetization Reminder",
        body: `Hello ${photographerName}, we noticed you haven't monetized your account yet. Consider applying for monetization to earn from your work.`,
        type: "monetization",
        data: {
