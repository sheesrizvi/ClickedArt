...
const createReferral = asyncHandler(async (req, res) => {
  ...
  if (photographer) {
    ...
    if (!photographerData)
      return res.status(400).send({ message: "Photographer not exist" });
  }
...
const getAllReferrals = asyncHandler(async (req, res) => {
  ...
  const result = await Promise.all(
    referrals.map(async (ref) => {
      const userCount = await User.countDocuments({ referralcode: ref.code });
      ...
      return {
        ...ref.toJSON(),
        totalUsers: userCount + photographerCount,
      };
    })
  );
...
const getReferralByPhotographer = asyncHandler(async (req, res) => {
  ...
  const referrals = await Referral.find({ photographer })
    .populate("photographer")
    .populate("users")
    .sort({ createdAt: -1 });
  res.status(200).send(referrals);
...
const getActiveReferralByPhotographer = asyncHandler(async (req, res) => {
  ...
  if (!referrals || referrals.length > 0) {
    return res
      .status(400)
      .send({ message: "No Referral Code found for photographer" });
  }
...
const getUsersUsingReferral = asyncHandler(async (req, res) => {
  const { code } = req.query;
  const users = await User.find({ referralcode: code }).select(
    "-password -pushToken"
  );
  const photographers = await Photographer.find({ referralcode: code }).select(
    "-password -pushToken"
  );
  ...
  if (!users) {
    return res
      .status(404)
      .json({ message: "No users found for this referral code" });
  }
