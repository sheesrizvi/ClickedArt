...
//       const groupedOrders = orderItems.reduce((acc, item) => {
//       const model = item.imageInfo?.imageModel;
//       const price = item.imageInfo?.price || 0;

//       if (model === "ImageVault" && price > 0) {
//         const photographerId = item.imageInfo?.photographer || "unknown";
//         if (!acc[photographerId]) {
//           acc[photographerId] = [];
//         }
//         acc[photographerId].push(item);
//       } else if (model === "CustomUploadImageVault" && price > 0) {
//         if (!acc["custom-upload"]) {
//           acc["custom-upload"] = [];
//         }
//         acc["custom-upload"].push(item);
//       } else if (item.subTotal > 0) {
//         const printKey = `print-${acc.nextPrintId || 1}`;
//         acc[printKey] = [item];
//         acc.nextPrintId = (acc.nextPrintId || 1) + 1;
...
//    let orderType = null;
//     if (updatedItems.every((item) => item.imageInfo?.imageModel === "CustomUploadImageVault")) {
//       orderType = "custom-upload";
//     } else if (updatedItems.every((item) => item.imageInfo?.imageModel === "ImageVault")) {
//       orderType = "platform";
//     } else {
//       throw new Error("Mixed image types detected in a single order group. This should not happen.");
//     }
...
//   } else if (model === "CustomUploadImageVault") {
//     itemNames.push("Custom Upload Image");
//   }
// }
