...
const createMonetization = asyncHandler(async (req, res) => {
  ...
  res
    .status(201)
    .json({
      message: "Monetization request created successfully",
      monetization,
    });
});
...
const getMonetizationByPhotographerId = asyncHandler(async (req, res) => {
  ...
  const results = await Monetization.find({
    $or: [
      {
        panNumber,
      },
    ...
    photographer: { $ne: photographerId },
  }).populate("photographer");
...
const updateMonetizationStatus = asyncHandler(async (req, res) => {
  ...
  if (status === "rejected") {
    ...
    // });
    await sendNotificationToUser({
        userId: monetization.photographer._id,
        userType: "Photographer",
        title: "Monetization Status Updated",
        body: `Your monetization request has been ${status}.`,
        type: "monetization",
        data: {
            url: `clickedartartist://monetize-account`,
        },
    });
  }
...
const deleteMonetizationRequest = asyncHandler(async (req, res) => {
  ...
  res
    .status(200)
    .json({ message: "Monetization request deleted successfully" });
});
...
const updateMonetization = asyncHandler(async (req, res) => {
  ...
  if (passbookOrCancelledCheque)
    monetization.passbookOrCancelledCheque = passbookOrCancelledCheque;
  ...
  if (businessAccount) {
    if (businessAccount.gstCopy)
      monetization.businessAccount.gstCopy = businessAccount.gstCopy;
    if (businessAccount.firmPan)
      monetization.businessAccount.firmPan = businessAccount.firmPan;
    if (businessAccount.firmGstCertificate)
      monetization.businessAccount.firmGstCertificate =
        businessAccount.firmGstCertificate;
    if (businessAccount.gstNumber)
      monetization.businessAccount.gstNumber = businessAccount.gstNumber;
  }
  ...
  res
    .status(200)
    .json({
      message: "Monetization request updated successfully",
      monetization,
    });
});
...
const getPendingMonetizations = asyncHandler(async (req, res) => {
  ...
  const [monetizations, totalDocuments] = await Promise.all([
    Monetization.find({ status: "pending" })
      .populate("photographer")
      .sort({ createdAt: -1 })
      .skip((pageNumber - 1) * pageSize)
      .limit(pageSize),
