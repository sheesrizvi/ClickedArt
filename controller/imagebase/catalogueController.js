...
const createCatalogue = asyncHandler(async (req, res) => {
  ...
  if (catalogueExist) {
    return res.status(400).send({
      message: "Catalogue already exist with same name for photographer",
    });
  }
  ...
  const uploadedCatalogueCount = await Catalogue.countDocuments({
    photographer,
  });
  ...
  if (uploadedCatalogueCount >= catalogueLimit) {
    return res
      .status(400)
      .send({ message: "Catalogue Uploaded Limit reached for this plan" });
  }
  ...
  res
    .status(200)
    .send({ message: "Catalogue created Successfully", catalogue });
});
...
const getCatalogueByPhotographer = asyncHandler(async (req, res) => {
  ...
  const catalogues = await Catalogue.find({ photographer })
    .sort({ order: 1 })
    .populate("photographer")
    .populate({
      path: "images",
...
const updateCatalogue = asyncHandler(async (req, res) => {
  const { catalogueId, name, description, photographer, images, order } =
    req.body;
  ...
  res
    .status(200)
    .send({ message: "Catalogue updated successfully", catalogue });
});
